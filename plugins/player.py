...
async def yplay(_, message: Message):
    ...
    if message.audio:
        ...
        if message.reply_to_message:
            ...
            d=await message.reply_text("No me diste nada para Iniciar. Envíame un archivo de audio o responde / reproduce un archivo de audio.")
            await mp.delete(d)
            await mp.delete(message)
            return
    ...
    if type=="audio":
        if round(m_audio.audio.duration / 60) > DURATION_LIMIT:
            d=await message.reply_text(f"❌Audios más largos que {DURATION_LIMIT} minuto (s) no se permiten, el audio proporcionado es {round(m_audio.audio.duration/60)} minuto(s)")
            await mp.delete(d)
            await mp.delete(message)
            return
        if playlist and playlist[-1][2] \
                == m_audio.audio.file_id:
            d=await message.reply_text(f"{emoji.ROBOT} Ya agregado en la lista de reproducción")
            await mp.delete(d)
            await mp.delete(message)
            return
        ...
        elif not LOG_GROUP and message.chat.type == "supergrupo":
    ...
    if type=="youtube" or type=="query":
        ...
        if int(duration) > DURATION_LIMIT:
            k=await message.reply_text(f"❌ Vídeos de más de {DURATION_LIMIT} minuto (s) no permitido, el video proporcionado es {duration} minuto(s)")
            await mp.delete(k)
            await mp.delete(message)
            return
...
async def deezer(_, message):
    ...
    if " " in message.text:
        ...
        k=await message.reply_text("No me diste nada para iniciar use / dplay <nombre de la canción>")
        await mp.delete(k)
        await mp.delete(message)
        return
    user=f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
...
async def skip_track(_, m: Message):
    ...
    if len(m.command) == 1:
        ...
        if LOG_GROUP:
            await mp.send_playlist()
        elif not LOG_GROUP and m.chat.type == "supergrupo":
...
async def join_group_call(client, m: Message):
    ...
    if group_call.is_connected:
        k=await m.reply_text(f"{emoji.ROBOT} BOt ya unido al chat de voz")
        await mp.delete(k)
        await mp.delete(m)
        return
    await mp.start_call()
    chat = await client.get_chat(CHAT)
    k=await m.reply_text(f"Se unió con éxito al chat de voz en {chat.title}")
    await mp.delete(k)
    await mp.delete(m)
...
async def leave_voice_chat(_, m: Message):
    group_call = mp.group_call
    if not group_call.is_connected:
        k=await m.reply_text("No se ha unido a ningún Chat voz todavía.")
        await mp.delete(k)
        await mp.delete(m)
        return
    playlist.clear()
    ...
    k=await m.reply_text("Salió del chat de voz")
    await mp.delete(k)
    await mp.delete(m)
...
async def stop_playing(_, m: Message):
    group_call = mp.group_call
    if not group_call.is_connected:
        k=await m.reply_text("Nada para detener.")
        await mp.delete(k)
        await mp.delete(m)
        return
    if 1 in RADIO:
    ...
    k=await m.reply_text(f"{emoji.STOP_BUTTON} Se detuvo")
    playlist.clear()
    await mp.delete(k)
    await mp.delete(m)
...
async def restart_playing(_, m: Message):
    group_call = mp.group_call
    if not group_call.is_connected:
        k=await m.reply_text("No hay nada que reproducir para repetir")
        await mp.delete(k)
        await mp.delete(m)
        return
    if not playlist:
        k=await m.reply_text("Lista de reproducción vacía.")
        await mp.delete(k)
        await mp.delete(m)
        return
...
async def pause_playing(_, m: Message):
    group_call = mp.group_call
    if not group_call.is_connected:
        k=await m.reply_text("Nada reproduciendo para pausar.")
        await mp.delete(k)
        await mp.delete(m)
        return
...
async def resume_playing(_, m: Message):
    if not mp.group_call.is_connected:
        k=await m.reply_text("Nada se detuvo para reanudar.")
        await mp.delete(k)
        await mp.delete(m)
        return
    mp.group_call.resume_playout()
    k=await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} Reanudado",
                               quote=False)
    await mp.delete(k)
    await mp.delete(m)
...
async def mute(_, m: Message):
    group_call = mp.group_call
    if not group_call.is_connected:
        k=await m.reply_text("Nada tocando para silenciar.")
        await mp.delete(k)
        await mp.delete(m)
        return
...
async def unmute(_, m: Message):
    ...
    k=await m.reply_text(f"{emoji.SPEAKER_MEDIUM_VOLUME} No silenciado")
    await mp.delete(k)
    await mp.delete(m)
...
async def set_vol(_, m: Message):
    group_call = mp.group_call
    if not group_call.is_connected:
        k=await m.reply_text("Aún no se ha unido a ningún chat Voz.")
        await mp.delete(k)
        await mp.delete(m)
        return
    if len(m.command) < 2:
        k=await m.reply_text('Olvidaste pasar volumen (1-200).')
        await mp.delete(k)
        await mp.delete(m)
        return
...
async def notforu(_, m: Message):
    k=await m.reply("Quien diablos eres?.")
    await mp.delete(k)
    await mp.delete(m)
...
async def not_chat(_, m: Message):
    buttons = [
        ...
        [
            InlineKeyboardButton('👨‍💻 Developer', url='https://t.me/Arisel_Fdez'),
        ]
        ]
    k=await m.reply("<b>No puedes usar este bot en este grupo, para eso tienes que hacer tu propio bot desde el [Soure Code 👈](https://github.com/211119-Arisel-Fernandez/FdezMusic.git) below.</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
    await mp.delete(m)
